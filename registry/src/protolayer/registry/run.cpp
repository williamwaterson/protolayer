...
#include "protolayer/protolayer-registration.h"


int main()
{
  std::string fileName("etc/registry-config.xml");
  ...
  protolayer::registration::Service* registrationService = 0;
  try {
    const XMLCh* elementName = xercesc::XMLString::transcode("ontoservice");
    xercesc::DOMNodeList* elements = document->getElementsByTagName(elementName);
    xercesc::DOMElement* element = dynamic_cast <xercesc::DOMElement*> (elements->item(0));

    registrationService = new protolayer::registration::Service(*element);
  ...
  adminServer->add(*registrationService);
