...
void LoftingLevel::evaluatePositions(Mesh& subdivision)
{
...
void LoftingLevel::subdivideMesh(Mesh& subdivision)
{
  for (int i = 0; i < subdivision.getNumberVertices(); i++) {
    Vertex& vertex = subdivision.getVertex(i);
...
HalfEdge& LoftingLevel::getHalfEdge(Vertex& from,
					 Vertex& to)
{
  HalfEdge* result = 0;
...
Vertex& LoftingLevel::createVertex(Vertex& copyFromVertex)
{
...
LoftingLevel::LoftingLevel(Mesh& mesh)
{
  subdivideMesh(mesh);
...
Mesh& LoftingLevel::getMesh()
{
  return _subdivision;
}


const Mesh& LoftingLevel::getMesh() const
{
  return _subdivision;
}
