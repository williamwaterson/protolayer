...
public:
  ...
  void testCompletedAttributesCase1()
  {
    ...
    //    assertTrue(type->getScopedName() == "Chemistry.Atom");
  ...
  void testXdrCaseA()
  {
    ...
    ontologydto::TypeDTO* typeDTO = new ontologydto::TypeDTO(*_typeElement);
    Type* encodingType = new Type(*typeDTO);
    ...
    //    Type decodingType(istream);
    // istream.close();

    //    remove("testfile");

    //    assertTrue(decodingType.getName() == "Atom");
    //    assertTrue(decodingType.getScopedName() == "Chemistry.Atom");
